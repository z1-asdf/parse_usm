...
void decrypt_video_packet(std::vector<char> &packet,
  ...
  for (size_t i = 0x100; i < encrypted_part_size; ++i) {
    size_t now_bit = 0x20 + (i % 0x20);
    data[i] ^= rolling[now_bit];
    rolling[now_bit] = data[i] ^ video_key[now_bit];
  }
...
struct UTF_Info {
  ...
  */
  ...
  UTF_Info(const std::vector<char> &data) {
    ...
    std::memcpy(magic, ptr, 4);
    ptr += 4;
    payload_size = ptr_to_uint<uint32_t>(ptr);
    data2_offset = ptr_to_uint<uint32_t>(ptr);
    str_stream_offset = ptr_to_uint<uint32_t>(ptr);
    byte_stream_offset = ptr_to_uint<uint32_t>(ptr);
...
bool parse_flag(const uint8_t &flag, bool &enable_dataII, UTF_DATA_TYPE &type,
  ...
  const size_t uses[] = {1, 1, 2, 2, 4, 4, 8, 8, 4, 8, 4, 8};
...
struct UTF_Data {
  ...
  UTF_Data(const uint8_t *&ptr) {
    ...
    name_offset = ptr_to_uint<uint32_t>(name_ptr);
...
std::string char_to_string(std::vector<char> &vec, size_t pos) {
  if (pos >= vec.size())
    return "out_of_bound: " + std::to_string(pos) + " / " +
...
std::string char_to_string(const uint8_t *ptr, const uint8_t *buffer_end) {
...
bool getOutput_payload_data_for_ivf(ChunkPayload &payload,
  ...
  while (offset < payload.data.size()) {
    ...
    uint32_t frame_size =
        ptr_to_uint<uint32_t>(frame_header_ptr, ENDIAN::LITTLE);
    uint64_t timestamp =
        ptr_to_uint<uint64_t>(frame_header_ptr, ENDIAN::LITTLE);
    ...
    if (enc) {
      ...
      end_ptr = reinterpret_cast<const uint8_t *>(payload.data.data()) +
                payload.data.size();
    }
    VP9FrameContext context;
    parse_vp9_uncompressed_header(preview_ptr, end_ptr, output, context);
...
bool getOutput_payload_data_for1(ChunkPayload &payload, std::ostream &output) {
  ...
  const uint8_t *str_stream_ptr =
      reinterpret_cast<const uint8_t *>(data.data()) + 8 +
      utf.str_stream_offset;
